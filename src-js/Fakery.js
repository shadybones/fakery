...
/**
 * State is just a name->value pairing which represents the current state you are monitoring,
 * but with convienent methods.
 *
 * @constructor
 * @param {Object} currentState
 * @throws {Error} if the currentState exists but is not an object
 */
State = Fakery.State = function State(currentState){
    this.stateDef = {};
    this.addToState(currentState || {});
};
/**
 * Adds or replaces a name-value pair to the current monitor state
 * @param {String} property
 * @param {*} newValue
 * @throws {Error} When missing parameter property.
 */
State.prototype.changeState = function(property, newValue){
    if(!property) throw new Error("invalid or missing argument");
    this.stateDef[property] = newValue;
};
State.prototype.matches = function(state){
    return Fakery.util_eq(state,this.stateDef);
};
/**
 * Recursively adds all the properties of an item (object) to the state.
 * Items are what is compared to determine if states are equal.
 * If the item is NOT an object, it MUST have a prefix (name), and forwards to the "changeState" method.
 * @param {*} obj Item to be added to the state (for state comparison)
 * @param {String=} prefix The item's name if not an Object, else adds a prefix to the name of the item.
 *                         Generally this is the name of the property this item exists as the value of.
 * @throws {Error} When obj param is not an object AND prefix is empty or does not exist.
 */
State.prototype.addToState = function(obj, prefix){
    this.stack = [];
    prefix = prefix && prefix.replace(/([^\.])$/,"$1.") || "";
    this._internalAdds(obj,prefix);
    this.stack = [];
};
/**
 * Internal only.
 * @param {*} obj item to be added to the current state
 * @param {String} prefix
 * @private
 */
State.prototype._internalAdds = function(obj, prefix){
    if(typeof obj != "object"){
        if(!prefix) throw new Error("invlid arguments");
        this.changeState(prefix.slice(0,-1),obj);
    }else {
        if(this.stack.indexOf(obj)>=0) return;
        this.stack.push(obj);
        ...
        for(var prop in obj){
            this._internalAdds(obj[prop], prefix+prop+".");
        }
        prefix && this.changeState(prefix.slice(0,-1),obj.constructor.name);
    }
};
